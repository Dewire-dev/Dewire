...
const storeKanbanTaskForm = useForm({
  ...
  date_end: "",
  scheduled_time: "",
  kanban_list_id: props.kanban_list.id,
})
...
let tasks = computed (() => props.kanban_list.tasks);
...
const storeKanbanTask = async (isUpdate = false) => {
  if (isUpdate) {
  ...
  <DialogModal :isOverFlow="false" :show="addingTask" @close="closeTaskModal(true)">
    ...
    <template #content>
      ...
      <div class="mt-2">
        <label
            class="block mb-2 text-sm font-medium "
            :class="updateKanbanTaskForm.errors.scheduled_time ? 'text-red-500' : 'text-gray-900 dark:text-gray-300'">Temps
